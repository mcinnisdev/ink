...
```bash
npx inksite remove component pricing-table
...
3. **Restart the dev server.** Stop it with `Ctrl+C` and run `npx inksite serve` again.
...
1. **Make sure both processes are running.** The Tailwind watcher and Eleventy run concurrently via `npm run dev` (or `npx inksite serve`). If only Eleventy is running, your Tailwind source (`src/css/tailwind.css`) won't be compiled to `src/css/style.css`.
